...
TEST_F(FileCacheTest, SuccesfulListGraphs) {
    ...
    {
        DummyDirectory dir("FileCacheTest");
        ...
        db::FileCache cache = db::FileCache(graphPath, dataPath, clientWrapper);
...
TEST_F(FileCacheTest, UnsuccesfulListLocalGraphs) {
    {
        ...
        std::vector<fs::Path> graphs;
        chmod(graphPath.c_str(), 0000);
        auto res = cache.listLocalGraphs(graphs);
        chmod(graphPath.c_str(), 0755);
        ASSERT_FALSE(res);
        EXPECT_EQ(res.error().getType(), db::ErrorType::LIST_LOCAL_GRAPHS_FAILED);
    }
}
...
TEST_F(FileCacheTest, UnsuccesfulListLocalData) {
...

    {
        ...
        std::vector<fs::Path> folderResults;
        std::vector<fs::Path> fileResults;
        chmod(dataPath.c_str(), 0000);
        auto res = cache.listLocalData(fileResults, folderResults);
        chmod(dataPath.c_str(), 0755);
        ASSERT_FALSE(res);
        EXPECT_EQ(res.error().getType(), db::ErrorType::LIST_LOCAL_DATA_FAILED);
    }
}
