...
TEST(PropertyContainerTest, EdgeProperties) {
    const testing::TestInfo* const testInfo =
        testing::UnitTest::GetInstance()->current_test_info();
    ...
    std::unordered_map<EntityID, std::string> strings;

    auto t0 = Clock::now();
    for (size_t i = 0; i < 100000; i++) {
        strings.emplace(0, std::to_string(i));
    }
    auto t1 = Clock::now();
    std::cout << "Map " << getMilliseconds(t0, t1) << " ms" << std::endl;
