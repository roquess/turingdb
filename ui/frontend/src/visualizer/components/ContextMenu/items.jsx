...
export const ItemSelectAllBySameNodeTypeNoData = (props) => {
  const { actions, className, popoverProps } = props;
  const vis = useVisualizerContext();
  const nodeTypes = [
    ...new Set(
      vis
  ...
  const CustomButton = props.button
    ? (buttonProps) => props.button(buttonProps)
    : Button;
...
export const ItemSelectAllBySamePropertyNoData = (props) => {
  const { actions, className, popoverProps } = props;
