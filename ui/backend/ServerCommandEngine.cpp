...
#include "ReactPrototypeDevCommand.h"
...
void ServerCommandEngine::run(const std::vector<std::string>& dbNames) {
    ...
    try {
        ...
        bioserverCommand = std::make_unique<BioserverCommand>(dbNames);
...
void ServerCommandEngine::runDev() {
    ...
    reactCommand = std::make_unique<ReactDevCommand>();
    reactCommand->runDev(_group);
#endif
}

void ServerCommandEngine::runPrototypeDev() {
#ifdef TURING_DEV
    BioLog::log(msg::INFO_STARTING_DEV_UI_SERVER());
    ...
    auto& reactCommand = _servers[(uint8_t)ServerType::REACT];
    reactCommand = std::make_unique<ReactPrototypeDevCommand>();
    reactCommand->runDev(_group);
#endif
}
