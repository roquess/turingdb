    ...
    for module in `cat ${basedir}/${project_name}/${dataset}/analysis/notebooks/.modules.txt`; do
        ...
        if [[ ${subset_specfic_params} == "true" ]]; then
            ...
            for subs in $(jq -r '.submission.subset_list | .[]' ${gigaConfig} ); do 
                ...
                jq -s '.[0] * .[1]' <( echo $tmp) \
                                    <( cat ${basedir}/${project_name}/${dataset}/analysis/notebooks/${module}/default_parameters.json ) > ${basedir}/${project_name}/${dataset}/analysis/notebooks/${module}/config_${project_name}_${dataset}_${subs}.json && echo -e "\t[DONE]."
            done
        else
            ...
            jq -s '.[0] * .[1]' <( echo $tmp) \
                                <( cat ${basedir}/${project_name}/${dataset}/analysis/notebooks/${module}/default_parameters.json ) > ${basedir}/${project_name}/${dataset}/analysis/notebooks/${module}/config_${project_name}_${dataset}.json && echo -e "\t[DONE].\n"
        fi
    done
fi
